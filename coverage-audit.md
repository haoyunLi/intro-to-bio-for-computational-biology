...
| 机制动效 | `motion-lab.html` 保留生物机制 8 场景 42 步、数据机制 8 场景 37 步。章节页只在存在专属场景时显示图解；原先给所有章节自动生成的通用三步数据动画已移除。新增 `genetics-lab.html` 的 enhancer、VAF 和 signaling dynamics 模型要求读者主动改变 causal variable。 |
...
| 原有 69 章 | “实验信号 / 文件与单位 / 推断边界”卡片 | 每学一个生物概念，都能说出相应数据从何而来及哪些结论不能直接推出。 |
