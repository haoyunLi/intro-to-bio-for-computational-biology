...
复查日期：2026-09-20。当前网站有 10 门课、69 章、335 个教学小节、143 个教材来源条目，共 8 个 HTML 页面。本轮逐章复查全部教材、12 课复习和独立交互练习，把“能否读懂数据”作为优先验收标准；另新增 16 个原创机制动效、74 个步骤。网站是逐步扩展的教学项目；目前的部分课程仍是系统导论的深度，尚不能等同于几门完整本科教材。
...
| 机制动效 | 新增 `motion-lab.html` 和播放器：生物机制 8 场景 42 步、数据机制 8 场景 32 步。37 章有详细机制场景，全部 69 章有三步数据阅读导览；每场景附逐帧解释、教学假设和来源。播放器支持播放/暂停、前后帧、进度拖动、倍速、尊重减少动态效果设置、离屏暂停及移动端横向图容器。Foundations 增逐碱基/逐 codon 与 bulk 比例扫描；Data Lab 增 SBS 加入→成像→解除阻断以及柱、tile 等对象变化。 |

### 验证范围

- 原有 7 个 HTML 页面及 69 章内容的本地文件、锚点、图片 alt、重复 ID 和前置知识依赖图检查通过；所有教材引用键有效。新增动效页使全站现有 8 个 HTML 页面。
- 在浏览器遍历 69 章，检查桌面与 390 px 手机排版；12 课复习也逐课检查。未发现整页横向溢出；宽表和示意图在自己的容器内横向滚动。
- 实际完成 Foundations 的序列输入/提前终止、细胞比例与 TPM 切换，Data Lab 八个练习、真实单文件五步和队列六步；检查错误反馈、步骤解锁与重置。浏览器检查未记录到 console error/warning。
...
- 动效页已在浏览器逐帧遍历 16 个场景、74 个步骤，并检查自动播放结束、暂停、倍速、进度拖动和手机端无整页横向溢出。首页、教材、真实单文件页与真实队列页的动效集成已检查；Foundations / Data Lab 的播放、暂停、重置、离屏暂停和计算结果也已完成回归。
...
| 真实队列页 | 2 位 TCGA-BRCA 病例、4 份已核验文件，六步做去重、筛选、配对与量纲判断 | 把单文件读法扩展到小型真实 cohort，同时明确其样本量不足以支持显著性或临床结论。 |
...
4. **从小型真实队列走向完整研究。** 现在已用固定公开 GDC RNA 文件解释单文件字段，也用两位病例的四份同流程文件练习去重、纳排、按 case 配对、raw/TPM 选择与结论边界。下一步仍需更大且预先定义的 cohort、样本 QC、临床表 join、缺失模式、批次检查及差异表达设计，并提供可复现脚本。数据版本和流程应固定并引用 [GDC mRNA 管线](https://docs.gdc.cancer.gov/Data/Bioinformatics_Pipelines/Expression_mRNA_Pipeline/)与 [GEO 数据说明](https://www.ncbi.nlm.nih.gov/geo/info/overview.html)。
5. **影像与空间数据的物理测量。** 进一步讲染色、光学分辨率、像素到微米的校准、切片配准、分割误差、细胞邻域统计及其独立样本层级。现有专题解释了平台与输出，但还缺完整的影像误差传播练习。
6. **计算推断与统计基础。** 为初学者增加概率模型、测量误差、multiple testing、effect size、批次混杂、训练/验证/测试划分，以及“模型预测”和“生物机制解释”之间的边界；用同一份小数据贯穿这些步骤。

## 以“理解数据”为标准的后续验收

网站已能让读者在站内完成单文件层的观测对象、行列、count、零值、来源与推断边界判断，以及两病例真实子集的去重、筛选与配对。尚未经过真人零基础可用性测试；也尚未把**规模足够且有完整临床信息的真实 cohort** 从 manifest、样本纳排、临床 join 一路做到统计结果。下一轮优先让读者独立完成这些任务：
...
建议下一阶段按 **真实 cohort 与数据连接 → 细胞机制与实验判读 → 影像和空间误差 → 统计推断** 扩写；每一阶段都配原始文件、结构图、手算例题和能暴露常见误解的检查题。
