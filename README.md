...
首页提供一条以读懂数据为终点的九步路线。`foundations-lab.html` 用三个可操作模型解释 DNA→RNA→蛋白、bulk 组织混合和 raw count/CPM/TPM；`data-lab.html` 提供八个虚构教学数据练习，包括 FASTQ read→比对→gene count 和 0 / NA / QC 失败判读。`motion-lab.html` 汇集 **16 个原创机制动效、74 个步骤**：8 个生物机制场景共 42 步，8 个数据机制场景共 32 步；37 章教材有对应详细机制图，其余章节也可使用三步数据阅读导览。`real-data-walkthrough.html` 用一份**真实公开的 TCGA-BRCA STAR Counts 文件**做五步读表判断；`real-cohort-lab.html` 再用四份公开文件组成两病例的配对教学子集，练习 case/sample/file 去重、样本筛选、join、零值、raw/TPM 和推断边界。原来的 12 课保留作快速复习。肿瘤多组学是主线，也覆盖 Visium HD、Xenium、CODEX、enhancer、splicing、单细胞及医学影像。
...
原有 7 个页面、69 章教材与 12 课复习已完成内容和交互复核；本轮加入第 8 个页面 `motion-lab.html`。动效页的 16 个场景、74 个步骤已在浏览器逐帧遍历，并检查播放自动结束、暂停、倍速、进度拖动与手机端无整页横向溢出。新增动效在首页、教材、真实单文件页与真实队列页的集成，以及 Foundations / Data Lab 的播放、暂停、重置和计算结果也已完成回归；详细范围见 [`coverage-audit.md`](coverage-audit.md)。
...
- Data Lab 使用虚构的教学数据，交互展示 case/sample join、raw count/CPM、VCF/BED 坐标、tile/患者级拆分、SBS 循环、三条 FASTQ read 的质量/比对/计数路径、五种表达尺度，以及已测零值、panel 未测、QC 失败和待核查四种状态；每个练习都链接官方格式或方法文档。
...
- 真实队列练习有意选择 TCGA-BRCA 的 2 位病例，每位各有一份 Primary Tumor 与 Solid Tissue Normal RNA-seq STAR Counts 文件，共 4 份文件。四份完整 TSV 的 file size 与 MD5 均经下载核验；[站内快照](data/gdc-tcga-brca-paired-star-counts-snapshot.json)列出 UUID、获取时间、共同 workflow version、官方元数据与每份文件 4 个真实基因行。这个小子集用于练习数据关系与读数含义，没有足够的独立病例做统计、因果或临床推断。
...
- `motion-lab.html`、`bio-motion.css`、`bio-motion.js`、`bio-motion-biology.js`、`bio-motion-data.js`：16 个机制动效、74 个步骤及全教材数据导览
- `foundations-lab.html`、`.css`、`.js`：三个互动生物与数据模型；新增序列逐碱基/逐 codon 播放与 bulk 比例扫描
- `data-lab.html`、`data-lab.css`、`data-lab.js`、`data-lab-mapping.js`、`data-lab-missingness.js`：八个交互式数据练习；SBS 逐轮加入/成像/解除阻断，以及柱和 tile 变化动效
- `real-data-walkthrough.html`、`.css`、`.js`：真实 GDC 文件五步练习
- `real-cohort-lab.html`、`.css`、`.js`：四份真实 GDC 文件组成的六步小队列练习
