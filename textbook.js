(() => {
  ...
  function renderCurriculum() {
    ...
    $('#book-scope').textContent=`UIUC 5 阶段 · ${courses.length} 门课 · ${chapters.length} 章；Genetics、Metabolism、Immunology 深入。`;
  }
  ...
  function renderChapter(id, scroll = false) {
    ...
    const guideTerms=(chapter.terms||[]).slice(0,3);
    const chapterGuide=`<section class="understanding-guide" aria-label="本章零基础阅读导航"><header><span>ZERO-BACKGROUND MAP</span><h2>先把本章拆成四个问题</h2></header><div class="understanding-grid"><section><b>01 · OBJECT</b><h3>正在讨论什么？</h3>${guideTerms.length?`<dl>${guideTerms.map(item=>`<div><dt>${escape(item[0])}</dt><dd>${escape(item[1])}</dd></div>`).join('')}</dl>`:'<p>先确定 biological object 与观察尺度。</p>'}</section><section><b>02 · MECHANISM</b><h3>它怎样一步步发生？</h3><ol>${chapter.sections.slice(0,4).map(item=>`<li>${escape(item[0])}</li>`).join('')}</ol></section><section><b>03 · EVIDENCE</b><h3>什么数据才算证据？</h3><p>区分 abundance、location、activity 与 flux；同一个词在不同 assay 中可能不是同一个测量。</p></section><section><b>04 · BOUNDARY</b><h3>结论能走多远？</h3><p>每次从 observation 走向 mechanism，都写出 alternative explanation 与下一项可区分它们的 perturbation。</p></section></div></section>`;
